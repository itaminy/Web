...
if ($method === 'POST' && !$user_id) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO users (login, password_hash, full_name, phone, email, birth_date, gender, biography, contract_accepted) VALUES (:login, :password_hash, :full_name, :phone, :email, :birth_date, :gender, :biography, 1)");
        $stmt->execute([
            ':login' => $login,
        ...
        sendResponse(['success' => true, 'login' => $login, 'password' => $password, 'profile_url' => "/edit.php?id=$new_user_id"], 201, $format);
    } catch (PDOException $e) {
